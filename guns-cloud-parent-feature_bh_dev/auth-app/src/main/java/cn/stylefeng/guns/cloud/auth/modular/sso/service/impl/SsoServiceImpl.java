...
import cn.stylefeng.guns.cloud.auth.modular.sso.enums.CodeEnum;
import cn.stylefeng.guns.cloud.auth.modular.sso.enums.LoginTypeEnum;
...
import cn.stylefeng.guns.cloud.auth.modular.sso.security.exception.SecurityException;
import cn.stylefeng.guns.cloud.auth.modular.sso.service.*;
import cn.stylefeng.guns.cloud.auth.modular.sso.util.CookieUtil;
...
public class SsoServiceImpl implements SsoService {
    ...
    public String ssoLogin(SsoLoginReq ssoLoginReq) {
        ...
        if (ObjectUtil.isNotEmpty(ssoLoginReq.getCode())) {
    ...
    @Override
    public String ssoToken(SsoLoginReq ssoLoginReq) {
        ...
        //统一认证平台重定向返回地址
        return ssoLoginWithToken(ssoLoginReq,client);
    }
    ...
    private String ssoLoginWithToken(SsoLoginReq ssoLoginReq, AuthClient client) {
        LoginUser loginUser = authService.getLoginUser(ssoLoginReq.getCode());

        // 查看token是否过期，过期重定向回去获取code
        if (loginUser == null) {
            return null;
        }
        return afterLoginGetToken(client, ssoLoginReq, loginUser);
     ...
     *获取Token
     */
    private String afterLoginGetToken(AuthClient authClient, SsoLoginReq ssoLoginReq, LoginUser loginUser) {
        TokenInfo tokenInfo = sessionService.create(authClient, loginUser);
        authLoginLogService.loginLog(HttpContext.getRequest(), tokenInfo);
